#需要QPython Plus 3.9.4或以上，旧版QPython均无法正常运行
#需要提前执行documentTreeShowOpen
from .BaseWindow import *
from os import environ
from os.path import realpath
def listdir(f):
    f=esla('documentFileListFiles',f)
    if f==None:
        return ()
    return sorted(f)
def isdir(f):
    return esla('documentFileIsDirectory',f)
...
def ListDocumentFile(Title='选择一个文件(夹)',Path=SD,FileName=''):#主函数
#documentFile文件(夹)选择器(标题,初始路径,初始文件名)
    MainScreen.sPath=d=MainScreen.Path=PathStrip(Path)
    while True:
        try:
            l=listdir(d)
            if not l and d in defaultRoot:
                raise
        except:
            i=defaultRoot.copy()
            i=i.get(d)
            if i==None:
                ...
                l=rsla('getSdCardPaths')+list(rsla('documentTreeListAll'))
...
def PathStrip(Path):
    ...
    if Path[-1:]!='/':
        Path+='/'
    return Path
__all__=('ListDocumentFile','droid')