...
class MainScreen(Layout):
    def on_show(self):
        ...
        for i in C:
            if i<-100:
                c=C[i]
                if type(c)!=str:
                    Text[i]=v.pop('Spin%s'%i)
                    Text[i].set_listitems(c)
                else:
                    Text[i]=v.pop('Title%s'%i)
            else:
    ...
    def conf(self,view,dummy):
        ...
        for i in C:
            if i<-100:
                if type(C[i])!=str:
                    k=int(t[i].selectedItemPosition)
                    if self.returnValue:
                        k=C[i][k]
                    j[i]=k
            else:
    ...
    def canc(self,view,dummy):
        ...
        FullScreenWrapper2App.close_layout()
CC=('确认','取消')
UP={#按照字典顺序决定显示顺序，与键值无关
    0:'用户名',
    -101:'出生日期',
    -102:list(range(1970,2022)),
    -103:list(range(1,13)),
    -104:list(range(1,32)),
    -99:('省份','xx省'),
    1:'密码',
...
def LoginMenu(Title='登录',Items=UP,returnValue=False,button=CC):#主函数
...
#项目组格式3：{……,-101:"选项组A"提示文本,-102:("选项组A-a"选项1,"选项组A-a"选项2,……),-103:("选项组A-b"选项1,"选项组A-b"选项2,……),……,-121:"选项组B"提示文本,-122:("选项组B-a"选项1,"选项组B-a"选项2,……),-123:("选项组B-b"选项1,"选项组B-b"选项2,……),……,0:"用户名"提示文本,1:"密码"提示文本,……}
#如果returnValue=True,按“确认”返回：{……,-102:"选项组A-a-选项α"字符串内容,-103:"选项组A-b-选项β"字符串内容,……,-122:"选项组B-a-选项γ"字符串内容,-123:"选项组B-b-选项δ"字符串内容,……,0:用户名,1:密码,……}
#如果returnValue=False,按“确认”返回：{……,-102:"选项组A-a-选项α"位置整数,-103:"选项组A-b-选项β"位置整数,……,-122:"选项组B-a-选项γ"位置整数,-123:"选项组B-b-选项δ"位置整数,……,0:用户名,1:密码,……}
    m=[XML[0]%Str2Xml(Title)]
    MainScreen.Contain=d=Items
